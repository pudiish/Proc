...
if (statusEl) statusEl.textContent = 'Loading face detection...';
    ...
    function init() {
        ...
        initializeFaceDetection().then(enabled => {
            interviewState.faceDetectionEnabled = enabled;
        });
        setupWindowVisibilityDetection();
    }
...
async function initializeFaceDetection() {
    try {
        // Show loading state
        document.getElementById('face-detection-status').textContent = 'Loading face detection...';

        // Load TensorFlow.js and Blazeface model dynamically
        ...
        if (typeof tf === 'undefined' || typeof blazeface === 'undefined') {
            ...
            document.getElementById('face-detection-status').textContent = 'Face detection unavailable';
            return false;
        }

        // Load the Blazeface model
        interviewState.faceDetector = await blazeface.load();
        console.log('Blazeface model loaded successfully');
        document.getElementById('face-detection-status').textContent = 'Face detection ready';
        return true;
    } catch (error) {
        console.error('Error loading face detection model:', error);
        document.getElementById('face-detection-status').textContent = 'Face detection limited';
        showError('Face detection features will be limited due to model loading error.');
        return false;
    }
}
...
// Face detection function using Blazeface
async function detectFaces() {
    if (!interviewState.faceDetectionEnabled || interviewState.isCameraOff || 
        !interviewState.stream || interviewState.faceDetectionRunning) {
        return;
    }

    try {
        interviewState.faceDetectionRunning = true;
        ...
        // Detect faces
        const predictions = await interviewState.faceDetector.estimateFaces(video, {
            returnTensors: false,
        ...
        if (predictions.length === 0) {
            addWarning('No face detected. Please ensure your face is visible in the frame.');
        } else if (predictions.length > 1) {
            addWarning('Multiple faces detected. Please ensure only you are visible in the frame.');
        } else {
            ...
            // Calculate face position percentages
            const faceCenterX = (box[0] + box[2]) / 2;
            const faceCenterY = (box[1] + box[3]) / 2;
            const xPercent = (faceCenterX / videoWidth) * 100;
            const yPercent = (faceCenterY / videoHeight) * 100;

            // Check if face is centered
            if (xPercent < 30 || xPercent > 70 || yPercent < 30 || yPercent > 70) {
                addWarning('Please center your face in the frame for better visibility.');
            }

            // Check mouth openness only if landmarks exist
            if (face.landmarks && face.landmarks.length >= 2) {
                try {
                    const upperLip = face.landmarks[0];
                    const lowerLip = face.landmarks[1];
                    
                    // Additional safety check
                    if (upperLip && lowerLip && upperLip.length >= 2 && lowerLip.length >= 2) {
                        const mouthOpenness = Math.abs(upperLip[1] - lowerLip[1]);
                        
                        // Simple threshold for mouth openness
                        if (mouthOpenness > 10) {
                            console.log('Mouth open detected - user is likely speaking');
                        }
                    }
                } catch (error) {
                    console.log('Error processing mouth landmarks:', error);
                }
            }
        }
    } catch (error) {
        console.error('Face detection error:', error);
        // Disable face detection after multiple errors
        if (interviewState.faceDetectionErrorCount++ > 3) {
            console.log('Disabling face detection due to errors');
            interviewState.faceDetectionEnabled = false;
            document.getElementById('face-detection-status').textContent = 'Face detection disabled';
        }
    } finally {
        interviewState.faceDetectionRunning = false;
    }
}
    ...
    function startDetectors() {
        // Face detection with Blazeface (if enabled)
        if (interviewState.faceDetectionEnabled) {
            interviewState.faceDetectionInterval = setInterval(() => {
                detectFaces();
            }, 3000); // Run every 3 seconds
        } else {
            console.log('Face detection disabled - using fallback behavior');
            // Fallback behavior when face detection isn't available
            interviewState.faceDetectionInterval = setInterval(() => {
                if (interviewState.isCameraOff) return;

                // Simple check if video is playing (very basic fallback)
                if (userVideo.readyState > 1 && userVideo.videoWidth > 0) {
                    // Video is playing - assume face is visible
                    console.log('Fallback face check: video appears active');
                } else {
                    addWarning('Camera not working properly. Please check your video feed.');
                }
            }, 5000);
        }
    ...
    function addWarning(message) {
        ...
        interviewState.warnings.push(warning);

        // Log warning to console
        console.log(`Warning: ${message} at ${warning.time.toLocaleTimeString()}`);
    ...
    async function endInterview() {
        ...
        stopDetectors();
        interviewState.results.windowFocusLostCount = interviewState.windowFocusLostCount;
    ...
    async function saveInterviewResults() {
        try {
            ...
            const resultsData = {
                ...
                startTime: interviewState.startTime.toISOString(),
                endTime: interviewState.endTime.toISOString(),
                duration: (interviewState.endTime - interviewState.startTime) / 1000, // in seconds
                questionsAnswered: interviewState.currentQuestionIndex,
                totalQuestions: interviewState.totalQuestions,
                warnings: interviewState.warningCount,
                score: interviewState.totalScore,
                answers: interviewState.answers,
                feedback: interviewState.questionFeedback
            };
    ...
    function calculateResults() {
        ...
        // Update state
        interviewState.totalScore = totalScore;
        interviewState.questionFeedback = questionFeedback;
    ...
    function setupWindowVisibilityDetection() {
        document.addEventListener('visibilitychange', () => {
            if (document.hidden) {
                // User switched to another tab or minimized the window
                interviewState.windowFocusLost = true;
                interviewState.windowFocusLostCount++;

                const warningMsg = `Focus lost (${interviewState.windowFocusLostCount}x). Please stay on the interview tab.`;
                addWarning(warningMsg);

                console.log('Window/tab focus lost');
            } else {
                interviewState.windowFocusLost = false;
                console.log('Window/tab focus regained');
            }
        });

        // Also detect window blur (for cases where visibilitychange doesn't fire)
